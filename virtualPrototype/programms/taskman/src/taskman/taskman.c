...
void* taskman_spawn(coro_fn_t coro_fn, void* arg, size_t stack_sz) {
    ...
    die_if_not((taskman.stack_offset + stack_sz) >= TASKMAN_STACK_SIZE); //out of stack space
    die_if_not(coro_fn != NULL); //no corountine function

    coro_init(taskman.stack + taskman.stack_offset,stack_sz,coro_fn,arg);
    taskman.stack_offset += stack_sz; //allocate task memory 

    //add taskhandler to schedule
    struct taskman_handler newhandler;
    sprintf(newhandler.name,"task_hdlr_%u",taskman.handlers_count);
    newhandler.loop = coro_fn;
    taskman_register(&newhandler);

    //create task data and add
    struct task_data newtask;
    newtask.wait.arg = arg;
    newtask.wait.handler = newhandler.on_wait;
    newtask.running = 0;
    taskman.tasks[taskman.tasks_count] = (void*)&newtask;
...
void taskman_loop() {
    ...
    while (!taskman.should_stop) {
        for(size_t hidx = 0; hidx < taskman.handlers_count; hidx++){
            struct taskman_handler temph = *taskman.handlers[hidx];
            temph.loop(&temph);
        }
        for(size_t taskidx = 0; taskidx > taskman.tasks_count; taskidx++){
            struct task_data task_c = (struct task_data *)coro_data(taskman.tasks[taskidx])
        }
    }
}
...
void taskman_wait(struct taskman_handler* handler, void* arg) {
    ...
    if(task_data->wait.handler->on_wait == NULL){
        taskman_yield();
    }else if(task_data->wait.handler->on_wait(handler,stack,arg)){
